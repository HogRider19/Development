...
Теперь рассмотрим, как мы можем встраивать частичные представления в обычные. Метод `Html.PartialAsync()` встраивает код частичного представления в обычное. Он является асинхронным и возвращает объект `IHtmlContent`, который представляет `html` содержимое и который обернут в объект `Task<TResult>`. В качестве параметра в метод передается имя представления:
