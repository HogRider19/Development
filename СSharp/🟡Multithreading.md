...
Монитор работает в двух режимах тонкой и толстой блокировки. Тонкая используется при наличии одного потока в секции, она использует набор счетчиков и атомарные инкременты. Толстая блокировка используется только при наличии конкуренции с другими потоками, она использует системную блокировку платформы: либо мьютекс, либо обработчики событий. Переход между тонкой и толстой блокировкой называется инфляцией блокировки. 
