...
После срабатывания исключения процессор передаёт управление в ядро, где запускается универсальный обработчик исключений. В `Linux` ядро преобразует аппаратный трап в сигнал, который доставляется тому же потоку, где случилось исключение программы. Пользователь может установить обработчик через `sigaction()`. Если обработчик не определён, по умолчанию процесс завершается или игнорирует сигнал.
